...
class Patreon {
    ...
    static async FetchPatrons(filters = ['active_patron'], pageSize = 1500, showSandboxPatrons = false) {
        let nextLink = `${this._URL}campaigns/${this._CampaignID}/members?include=user,currently_entitled_tiers&page[count]=${pageSize}&fields[member]=campaign_lifetime_support_cents,currently_entitled_amount_cents,email,full_name,is_follower,last_charge_date,last_charge_status,lifetime_support_cents,next_charge_date,note,patron_status,pledge_cadence,pledge_relationship_start,will_pay_amount_cents&fields[user]=social_connections&fields[tier]=title`;
    
        nextLink = this.CleanURL(nextLink);
    
        const Patrons = [];
    
        try {
            while (nextLink) {
                const fetchResponse = await axios_1.default.get(nextLink, {
                    headers: { Authorization: 'Bearer ' + this._AccessToken }
                });
                        ...
                        subscription: {
                            ...
                            currentEntitled: {
                                ...
                                tier: {
                                    ...
                                    title: tierInfo && tierInfo.attributes ? tierInfo.attributes.title : null, // Update here
                                },
        ...
        throw new Error('Fetch API Failed...' + error);
    }
}
